...
def extract_experts(
    ...
    expert_starts = []
    expert_opt_starts = []
    for i in range(iterations or DEFAULT_ATTACK_ITERATIONS):
        for s in config['trajectories']: #batch iteration  50 100 150 200
            expert_id = np.random.randint(config['experts'])
            trajectory = np.random.randint(config['min'], config['max']) + 1 # epoch
            expert_starts.append(expert_path.format(expert_id, trajectory, str(s)))
            if expert_path:
                expert_opt_starts.append(expert_opt_path.format(expert_id, trajectory, str(s)))
